import { UnderrunEvent } from "@ain1084/audio-worklet-stream/dist/events.js";
import type { FrameBufferWriter } from "@ain1084/audio-frame-buffer";
import { createFrameBufferContext } from "@ain1084/audio-frame-buffer/dist/frame-buffer-context.js";
import { ManualBufferWriteStrategy } from "@ain1084/audio-worklet-stream/dist/write-strategy/manual-strategy.js";
import { OutputStreamNode } from "@ain1084/audio-worklet-stream/dist/output-stream-node.js";

import type { DecodedPacket } from "../multiplayer/audio/packets";
...
const LEAD_BUFFER_FRAMES = 48000; // 1s at 48kHz

export class AudioLabPlayback {
  ...
  async enqueue(path: string, packet: DecodedPacket): Promise<void> {
    ...
    if (written < totalFrames) {
      remote.droppedFrames += totalFrames - written;
      // eslint-disable-next-line no-console
      console.warn(`AudioLabPlayback: dropping ${totalFrames - written} frames for ${path}`);
    }
    ...
    const bufferedFrames = Math.max(0, totalWritten - totalRead);

    remote.stats.bufferedAhead = bufferedFrames / remote.sampleRate;
  ...
  async #createRemote(path: string, packet: DecodedPacket): Promise<RemoteState> {
    ...
    const bufferContext = createFrameBufferContext({
      channelCount,
      frameCount,
    });
    const strategy = new ManualBufferWriteStrategy(bufferContext);
    const node = await OutputStreamNode.create(this.#context, bufferContext, strategy);
    const writer = strategy.writer;
